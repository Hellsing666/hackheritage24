...
document.addEventListener('DOMContentLoaded', function () {
    const articles = {
        ...
        4: {
            title: "Research & Findings", // Corrected spelling
    ...
    if (articleId && articles[articleId]) {
        ...
        buttons.forEach(button => {
            button.addEventListener('click', (event) => {
                ...
                // Show an alert (replace with your modal or other functionality)
                const techniqueName = button.previousElementSibling.previousElementSibling.textContent;
                alert(`Learn more about ${techniqueName}`); // Fixed template literal syntax
            });
        });
    }
});